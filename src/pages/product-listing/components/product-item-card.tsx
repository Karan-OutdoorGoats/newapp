...
const ProductItemCard = () => {
  ...
  return (
    <div
      className={`border-[1px] box-content ${isGrid
    ...
    >
      <div
        className={`relative overflow-clip md:p-2 img_container ${isGrid ? "" : "basis-[30%]"} `}
      >
        <Link to={"/product-page"} className="relative md:h-[280px] h-[unset] flex flex-row items-center justify-center overflow-hidden">
          <img
          ...
          <div className="absolute bottom-[12%] md:bottom-[10%] h-[20px]  flex flex-col justify-center left-[25%]  md:left-[28%]  origin-center rotate-[-33deg] z-[2]  bg-orange text-white font-OG-Regular w-full  py-[1px]">
            <p className="text-center uppercase text-[0.6rem] md:text-11">Athlete Reviewed</p>
          </div>
        </Link>
      ...
      <div
        className={`flex flex-col ${isGrid
          ? "items-center h-[200px] justify-center"
          : "py-2 justify-between basis-[70%] "
          }   px-2 product_desc `}
      >
        <div className={`${isGrid ? "flex flex-col items-start px-2" : "mt-5"} `}>
          <div className="flex flex-row items-center justify-between w-full">
            <p className=" text-[11px] uppercase font-OG-Medium text-lightTextColor basis-[70%] md:basis-[60%]">Baller Athletik </p>
            <div
              title="wishlist"
              onClick={() =>
                toast(
                  ...
                  {
                    position: screenSize.width > 600 ? "top-center" : "bottom-center",
                    autoClose: 1800,
                    // hideProgressBar: true,
                    // autoClose: false,
                    toastId: "added_wishlist",
          ...
          <div className="mt-2">
            <Link to={"/product-page"}>
              <p
                className={`text-sm font-[200] mb-1  ${isGrid ? "text-start" : "text-start text-pretty"
        ...
        <OGButton
          onClick={() => setViewProduct(true)}
          className={`uppercase ${isGrid ? "md:hidden group-hover:block w-full" : "block mb-5 w-[40%]"
